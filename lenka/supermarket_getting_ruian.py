...
# --- Konfigurace ---
INPUT_FILE = 'dataset_supermarket.json'  # Změň na název tvého NOVÉHO JSON souboru
OUTPUT_FILE = INPUT_FILE.replace(".json", "") + "_kod_obce.csv"

# Vstup - tvůj JSON soubor
# DATA_ITEMS_PATH = ['data', 'items']  # TATO PROMĚNNÁ UŽ NENÍ POTŘEBA
CITY_COL = 'city'
STREET_COL = 'street'
POSTAL_CODE_COL = 'postalCode'
ADDRESS_COL = 'address'  # Budeme primárně používat sloupec 'address'
...
def get_address_alternatives(data_item):
    ...
    address = data_item.get(ADDRESS_COL, "")
    city = data_item.get(CITY_COL, "")
    postal_code = data_item.get(POSTAL_CODE_COL, "")
    street = data_item.get(STREET_COL, "")

    city_plus_code = f"{city} {postal_code}".strip()
    simplified_address = "".join(char for char in address if not char.isdigit() and char != '/')
    simplified_address = simplified_address.replace("nám.", "").strip()

    street_city_code = f"{street} {city} {postal_code}".strip()

    alternatives = []
    if address:
        alternatives.append(('full_address', address))
    if street and city_plus_code:
        alternatives.append(('street_city_psc', f"{street.split(',')[0].strip()} {city_plus_code}")) # Vezmeme jen část ulice před čárkou
    if city_plus_code:
        alternatives.append(('city_psc_only', city_plus_code))
    if street:
        alternatives.append(('street_only', street.split(',')[0].strip())) # Opět jen část ulice
...
def match_address_to_city_code_from_json():
    ...
    try:
        ...
        data_items = json_data  # Přímo list s daty
    ...
    # Propojíme s původními daty
    original_df = pd.DataFrame(data_items)
